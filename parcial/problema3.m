...
function agregar_producto(conn, identificacion, nombre, cantidad, precio)
    try
        query = sprintf("INSERT INTO problema3 (identificacion, nombre, cantidad, precio) VALUES ('%s', '%s', %d, %f);", identificacion, nombre, cantidad, precio);
...
function actualizar_producto(conn, identificacion, nombre, cantidad, precio)
    try
        query = sprintf("UPDATE problema3 SET nombre = '%s', cantidad = %d, precio = %f WHERE identificacion = '%s';", nombre, cantidad, precio, identificacion);
        pq_exec_params(conn, query);
...
function eliminar_producto(conn, identificacion)
    try
        query = sprintf("DELETE FROM problema3 WHERE identificacion = '%s';", identificacion);
        pq_exec_params(conn, query);
