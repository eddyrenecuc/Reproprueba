...
function ingresar_sensor(conn)
    ...
    anio = input('Ingrese el anio del sensor: ');

    % Insertar datos en la base de datos
    query = sprintf("INSERT INTO problema6 (nombre, marca, anio) VALUES ('%s', '%s', %d);", nombre, marca, anio);
...
function visualizar_sensor(conn)
    ...
    query = sprintf("SELECT * FROM problema6 WHERE nombre = '%s';", nombre_sensor);
...
function mostrar_graficas(conn)
    % Consultar todos los datos de la tabla problema6
