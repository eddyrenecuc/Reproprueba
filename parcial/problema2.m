...
function conn = conectar_bd(identificacion, descripcion, monto, fecha)
    ...
    query = sprintf("INSERT INTO problema2 (identificacion, descripcion, monto, fecha) VALUES (%d, '%s', %d, '%s');", identificacion, descripcion, monto, fecha);
                ...
                case 4
                    if isempty(historial)
                        ...
                        if isempty(indice)
                            ...
                            conn = pq_connect(setdbopts('dbname', 'examenparcial1', 'host', 'localhost', 'port', '5432', 'user', 'postgres', 'password', 'eddyrene'));
                            update_query = sprintf("UPDATE problema2 SET monto = %d, fecha = '%s' WHERE identificacion = '%s';", nuevo_monto, nuevo_fecha, num2str(identificacion_modificar));
