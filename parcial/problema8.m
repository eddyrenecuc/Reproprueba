...
function ingresar_empresa(conn)
    try
        ...
        query = sprintf("INSERT INTO problema8 (nombre, actividad, pais, cantidad_empleados) VALUES ('%s', '%s', '%s', %d);", nombre, actividad, pais, cantidad_empleados);
...
function visualizar_empresa(conn)
    try
        ...
        query = sprintf("SELECT * FROM problema8 WHERE nombre = '%s';", nombre_empresa);
...
function mostrar_graficas(conn)
    % Consultar todos los datos de la tabla problema8
    query = "SELECT * FROM problema8;";
    result = pq_exec_params(conn, query);
