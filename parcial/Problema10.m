...
while true
    ...
    disp("4. Ver lista de problema10");
    ...
    try
        switch opcion
            case 1
                ...
                insert_query = "INSERT INTO problema10 (nombre, cantidad) VALUES ($1, $2);";
            ...
            case 2
                ...
                select_query = sprintf("SELECT * FROM problema10 WHERE id = %d;", id_producto);
                ...
                if ~isempty(result.data)
                    ...
                    update_query = "UPDATE problema10 SET cantidad = $1 WHERE id = $2;";
            ...
            case 3
                ...
                if ~isempty(result.data)
                    ...
                    delete_query = "DELETE FROM problema10 WHERE id = $1;";
            ...
            case 4
                % Ver lista de problema10
                select_all_query = "SELECT * FROM problema10;";
                result = pq_exec_params(conn, select_all_query);

                problema10 = result.data;

                if isempty(problema10)
                    disp("No hay problema10 en la base de datos.");
                else
                    disp("Lista de problema10:");

                    for i = 1:size(problema10, 1)
                        fprintf('Producto %d:\n', i);
                        fprintf('ID: %d\n', problema10{i, 1});
                        fprintf('Nombre: %s\n', problema10{i, 2});
