...
function ingresar_pelicula(conn)
    ...
    query = sprintf("INSERT INTO problema7 (nombre, genero, clasificacion) VALUES ('%s', '%s', '%s');", nombre, genero, clasificacion);
...
function recomendar_pelicula(conn)
    ...
    query = sprintf("SELECT nombre FROM problema7 WHERE genero = '%s';", genero);
